  ...
  testFiles.forEach(file => {
    ...
    it(`should parse the file ${file} ${isFaulty ? 'and fail' : 'without errors'}`, () => {
      ...
      try {
        ...
        if (isFaulty) {
          expect.fail(`Expected parsing to fail for faulty test ${file}, but it succeeded.`);
        }

        // Check if the parse tree is not null or undefined
        expect(parseTree).to.exist;
        expect(parseTree.topNode).to.exist;
      } catch (error) {
        if (!isFaulty) {
          expect.fail(`Parsing failed for ${file} with error: ${error.message}`);
        }
        // For faulty tests, we expect a failure, so do nothing
      }
    });
